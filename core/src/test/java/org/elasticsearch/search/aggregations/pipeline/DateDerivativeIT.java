...
public class DateDerivativeIT extends ESIntegTestCase {
    ...
    public void testSingleValuedFieldNormalised_timeZone_CET_DstStart() throws Exception {
        ...
        SearchResponse response = client()
                ...
                .addAggregation(dateHistogram("histo").field("date").interval(DateHistogramInterval.DAY)
                        .timeZone(timezone.getID()).minDocCount(0)
                        .subAggregation(derivative("deriv").setBucketsPaths("_count").unit(DateHistogramInterval.HOUR)))
                .execute()
                .actionGet();
    ...
    public void testSingleValuedFieldNormalised_timeZone_AsiaKathmandu() throws Exception {
        ...
        SearchResponse response = client()
                ...
                .addAggregation(dateHistogram("histo").field("date").interval(DateHistogramInterval.HOUR)
                        .timeZone(timezone.getID()).minDocCount(0)
                        .subAggregation(derivative("deriv").setBucketsPaths("_count").unit(DateHistogramInterval.MINUTE)))
                .execute()
                .actionGet();
