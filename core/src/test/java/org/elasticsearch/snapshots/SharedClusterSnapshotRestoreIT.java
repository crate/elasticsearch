...
public class SharedClusterSnapshotRestoreIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testRestoreTemplatesExplicitTest() throws Exception {
        Client client = client();
        ...
        logger.info("--> restore templates");
        RestoreSnapshotResponse restoreSnapshotResponse = client.admin().cluster().prepareRestoreSnapshot("test-repo", "test-snap").setTemplates("test-template").setWaitForCompletion(true).execute().actionGet();
        // We don't restore any indices here
        assertThat(restoreSnapshotResponse.getRestoreInfo().totalShards(), equalTo(0));
        ...
        logger.info("--> restore ALL templates");
        client().admin().cluster().prepareRestoreSnapshot("test-repo", "test-snap").setTemplates("_all").setWaitForCompletion(true).execute().actionGet();
        getIndexTemplatesResponse = client().admin().indices().prepareGetTemplates().get();
        assertIndexTemplateExists(getIndexTemplatesResponse, "test-template");
    }
    ...
    public void testRestoreTemplatesNotOverwritingExistingOnes() throws Exception {
        ...
        logger.info("-->  creating test template");
        assertThat(client.admin().indices().preparePutTemplate("test-template").setTemplate("te*").addMapping("test-mapping", "{\"test-mapping\" : {\"properties\" : {\"old_value\" : {\"type\" : \"integer\"}}}}").get().isAcknowledged(), equalTo(true));
        ...
        assertThat(client.admin().indices().preparePutTemplate("test-template").setTemplate("te*").addMapping("test-mapping", "{\"test-mapping\" : {\"properties\" : {\"value\" : {\"type\" : \"string\"}}}}").get().isAcknowledged(), equalTo(true));
        ...
        assertThat(getIndexTemplatesResponse.getIndexTemplates().get(0).getMappings().get("test-mapping").string(), equalTo("{\"test-mapping\" : {\"properties\" : {\"value\" : {\"type\" : \"string\"}}}}"));
    }
